...
class PNet(nn.Module):
    ...
    def __init__(self, pretrained=True):
        ...
        if pretrained:
            state_dict_path = os.path.join(os.path.dirname(__file__), 'PreTrain\pnet.pt')
...
class RNet(nn.Module):
    ...
    def __init__(self, pretrained=True):
        ...
        if pretrained:
            state_dict_path = os.path.join(os.path.dirname(__file__), 'PreTrain\\rnet.pt')
            state_dict = torch.load(state_dict_path)
            self.load_state_dict(state_dict)
...
class ONet(nn.Module):
    ...
    def __init__(self, pretrained=True):
        ...
        if pretrained:
            state_dict_path = os.path.join(os.path.dirname(__file__), 'PreTrain\onet.pt')
            state_dict = torch.load(state_dict_path)
            self.load_state_dict(state_dict)
...
class MTCNN(nn.Module):
    ...
    def detect_face(self,imgs,threshold = [0.7,0.8,0.9], factor = 0.709, device = 'cuda:0', minsize=300, ):
        ...
        if len(boxes) > 0:
            ...
            # This is equivalent to out = self.rnet(im_data) to avoid GPU out of memory.
